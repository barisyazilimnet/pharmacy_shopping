...
<?php
    if($_SESSION["uye_yetki"]>=$yetki & $_SESSION["uye_id"]!=$uye_id){
		...
		header("Refresh:3; url = http://localhost/pharmacy_shopping/login/yonetici.php?do=uye_islemleri", true, 303);
	}else {
		...
		if($sil){
			...
			header("Refresh:15; url = http://localhost/pharmacy_shopping/login/yonetici.php?do=uye_islemleri", true, 303);
...
<?php
		...
		header("Refresh:3; url = http://localhost/pharmacy_shopping/login/yonetici.php?do=anasayfa", true, 303);
    }
?>